...
def get_valid_images(list):
	...
	for image in list:
		...
		if ('.jpg' in image)  or ('.jpeg' in image) or ('.JPG' in image) or ('.png' in image) or ('.PNG' in image) or ('.GIF' in image) or ('.gif' in image):
			...
			if response.status_code == 200:
				...
				with open('products/static/products/images/'+ str(datetime.date.today()) +'-'+ image_name, "wb")  as f:
					...
					new_katlogg_images_url += "https://s3.us-east-2.amazonaws.com/katlogg-categories-image/product_images/" + str(datetime.date.today()) +'-'+ image_name + "|"
					random_number += 1
			else:
				print("response not 200: ", response.status_code)
		else:
			print("OOps!!! - Image is not JPG Format")
	return [new_list, new_katlogg_images_url]
