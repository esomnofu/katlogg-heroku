...
from products.models import Product, News, Unidentified
...
def news(arg, argz):
	...
	with open(os.path.join(settings.NON_STATIC_ROOT+argz, last)) as json_file:
		...
		for p in data['product']:
			...
			product_images_urls = p['images_url']
			product_store_name = p['store_name']
			product_gender	=	p['gender']
			product_sub_sub_categories = p['sub_sub']
			...
			if old_item is 0:
				...
				#A way of not adding services to DB based on tagging from Detailed View
				...
				old_item_current_price	= str(old_item_current_price).replace(',', '')
				#Make Price an Integer thus suitable for making Comparison
				old_item_current_price = float(old_item_current_price)


				#Format the New Incoming Product also to make suitable for comparison
				new_item_current_price = str(product_current_price).replace(',', '')
				new_item_current_price = float(new_item_current_price)
