	...
	def get_item_from_selected_links(self, link):
			...
			if ',' in self.product_filters_texts:
				...
				for each in each_filters_texts:
					...
					#Fix for drinks.ng and any other texts with (21) e.g Shampoo (21) instead of Shampoo
					if "(" in each:
						pos = each.find('(')
						each = each[0:pos]
			...
			while start < len(filters):
				self.filters_texts[filters[start]] = each_filters_texts[start]
				start += 1
			...
			for eachFilter in filters:
				...
				self.filters.add(eachFilter)

			# print("Filters Dictionary : ", self.filters_texts)
			...
			if self.project == "amaget.com":
				# print("We trying to get the Bad Guys!!!")
				links = tree.xpath(self.product_url)
				# print("All URLS under this category: ", links)
				for each in links:
					if "html" in each:
						amaget_links.append(each)
				# print("Valid Amaget Links on this Category are: ", amaget_links)
				for link in amaget_links:
				...
				links = tree.xpath(self.product_url)
				# print("URL is : ", self.starting_url)
				# print("All URLS under this category: ", links)
				for link in links:
					#Fix For DelphiMetals and Fashpa
					if ('http' not in link) and (self.project not in link):
						link = 'http://'+self.project+'/'+link
					self.items.add(link)
				self.start_page_number += 1
